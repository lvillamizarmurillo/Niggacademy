...
export default class Cursos {
    ...
    static async postCalificacion(req,res){
        if((!req.body.calificacion)||(req.body.calificacion < 0)||(req.body.calificacion > 5)) return res.status(400).send('Es necesario poner un numero del 1 al 5');
        const user = await traerUserLogin(req)
        const data = await calificacion.findOne({correoUsuario: user.correo,nombreCurso:req.params.curso})
        if(data) return res.status(400).send('Ya calificaste este curso.');
